...
export function LeaderboardTable({
  ...
  // Initialize sort from props (SSR-safe — parent reads searchParams via Suspense)
  const [sortKey, setSortKey] = useState<SortKey>(() => {
    const validKeys: SortKey[] = ["rank", "points", "max", "r64", "r32", "s16", "e8", "ff", "champ"];
    return validKeys.includes(initialSort as SortKey) ? (initialSort as SortKey) : "rank";
  });
  const [sortAsc, setSortAsc] = useState(() => {
    if (initialDir === "desc") return false;
    return true;
  });
      ...
      <table className="w-full text-sm">
        <thead className="sticky top-0 z-20 bg-surface-container">
          <tr className="border-b border-outline">
            <th className="w-8"></th>
            <th className={hdr} title="Current standing based on total points" onClick={() => toggleSort("rank")}><span className={hdrDotted}>Rank</span>{sortIcon("rank")}</th>
            ...
            <th className={hdr} title="Total points earned from correct picks" onClick={() => toggleSort("points")}><span className={hdrDotted}>Pts</span>{sortIcon("points")}</th>
            <th className={hdr} title="Maximum possible points if all remaining picks are correct" onClick={() => toggleSort("max")}><span className={hdrDotted}>Max</span>{sortIcon("max")}</th>
            <th className={hdr} title="Points earned in Round of 64 (10 pts per correct pick)" onClick={() => toggleSort("r64")}><span className={hdrDotted}>R64</span>{sortIcon("r64")}</th>
            <th className={hdr} title="Points earned in Round of 32 (20 pts per correct pick)" onClick={() => toggleSort("r32")}><span className={hdrDotted}>R32</span>{sortIcon("r32")}</th>
            <th className={hdr} title="Points earned in Sweet 16 (40 pts per correct pick)" onClick={() => toggleSort("s16")}><span className={hdrDotted}>S16</span>{sortIcon("s16")}</th>
            <th className={hdr} title="Points earned in Elite 8 (80 pts per correct pick)" onClick={() => toggleSort("e8")}><span className={hdrDotted}>E8</span>{sortIcon("e8")}</th>
            <th className={hdr} title="Points earned in Final Four (160 pts per correct pick)" onClick={() => toggleSort("ff")}><span className={hdrDotted}>FF</span>{sortIcon("ff")}</th>
            <th className={hdr} title="Points earned in Championship (320 pts per correct pick)" onClick={() => toggleSort("champ")}><span className={hdrDotted}>Champ</span>{sortIcon("champ")}</th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((b) => {
